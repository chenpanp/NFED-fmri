...
import random
from .Deit_20240711 import deit_small_distilled_patch16_224
...
class TransGeo(nn.Module):
    ...
    def forward(self, im_q, im_k, delta=None, atten=None, indexes=None, result = False):
        if atten is not None:
            return self.query_net(im_q), self.reference_net(x=im_k, atten=atten, indexes=indexes, result = result)
        else:
