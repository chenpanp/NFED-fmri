...
dataset_path = 'E:/Retinotopy/HAD-fmri-main/validation'
fmriprep_path = pjoin(dataset_path, 'derivatives', 'fmriprep')
support_path = 'E:/Retinotopy/HAD-fmri-main/validation/support_files'
...
for sub_dir in sub_dirs:
    ...
    files = [ _ for _ in os.listdir(pjoin(fmriprep_path, sub_dir)) if 'FD.xlsx' in _ \
                and int(re.findall(r'run-\d+', _)[0].split('-')[-1])<=5]
    files.sort()
    for file in files:
...
for i, sub_name in enumerate(FD_sum.keys()):
    ...
    for pc in parts['bodies']:
        pc.set_facecolor('#808080')  #'#81C6E8'
        pc.set_alpha(0.8)
    ...
    ax.vlines(i, data_min, data_max, color='k', linestyle='-', lw=1.5)
    ax.vlines(i, q1, q3, color='red', edgecolors='#E97777', linestyle='-', lw=3.6)
    ax.hlines(data_min, i - 0.2, i + 0.2, color='k', linestyle='-', lw=1.2)
    ax.hlines(data_max, i - 0.2, i + 0.2, color='k', linestyle='-', lw=1.2)
# define plot details
...
ax.set_xlim([-1, 65])
ax.plot([-1, 65], [0.5, 0.5], ls='--', color='#DD5353', lw=2)
ax.set_xticks(np.arange(0,65,1))
ax.set_xticklabels(['%02d' % (i+1) for i in np.arange(0,65,1)])
