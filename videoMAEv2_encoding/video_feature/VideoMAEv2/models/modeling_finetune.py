...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x)
        #
        feature_layer = deque(maxlen=self.depth)
        for i, blk in enumerate(self.blocks):
            ...
            if self.fc_norm is not None:
                feature_layer.append(self.fc_norm(x.mean(1)))
            else:
                feature_layer.append(self.norm(x[:, 0]))
    ...
    def forward(self, x):
        x = self.forward_features(x) # 1,3,16, 224，224
