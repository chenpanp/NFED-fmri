...
dataDir = '/home/xianchige/YZY/20220523data/sub03/session01/';
...
% ssname='litong_zc1';
 ssname='sub03';
% ssname='litong_dxh';
load('/home/xianchige/YZY/20220523data/sub03/session01/preprocess/result_for_prf_mean/retino_pRF_run_mean.mat');
...
data_mask = MRIread(['/home/xianchige/YZY/20220523data/sub03/sub03_' hemi{hm} '.Kastner2015Labels'  '.mgz']);
...
% labelname={'V1d','V3d','V2v','V3v','V4'};
[numlh, numrh] = cvnreadsurface(ssname, hemi, surf_tpe, surfsuffix, 'justcount',true);
betamask_result = logical(zeros(numlh+numrh,1));

        
%% show surface figure
angle_result = valstruct_create(ssname,surfsuffix,data); 
angle_result_c = valstruct_getdata(angle_result,hemi{hm});
...
[~,L,rgbimg1]=cvnlookupimages(ssname,angle_result_c,hemi{hm},ori, ...
    ...
    'surftype',surf_tpe,'clim',[0 350],'threshold',5);
...
% labelname={'V1d'};
%labelname={ 'LO'};
R1=zeros(length(angle_result_c),1); %
...
for i = 1:length(labelname)
    ...
    while(ishandle(h))
        ...
        tmprgb=bsxfun(@times,rgbimg1,.75*imgroi + .25);
        set(h,'cdata',tmprgb);
  ...
  % val = struct('data',<numlh+numrh x 1>,'numlh',numlh,'numrh',numrh);
    [rawimg,Lookup,rgbimg] = cvnlookupimages(ssname,angle_result,viewhemis,viewpt,Lookup,...
            'cmap', cmapang(64), ...
