...
clc
close all
data_dir='G:/cpp_data/sub03/session011/surfacedata';
...
hemis = {'lh','rh'};
runix=[1:6];   % in this session, the first 5 runs for regular stimuli
% load
data={};

% labelname={'V1d', 'V2d', 'V3d','V3a','V1v','V2v','V3v','V4'};
% voxelsnum_session1 = zeros(numel(runix),length(labelname));
...
for p=1:numel(runix)
    ...
    %         end
     ...
     num_conditions  = 64; % 5classes; 4 subcatigories  in each class; 3 images  for comparation in each catigory. 
     
     %% Create design matrix   
     
     load('G:/cpp_data/stim_label/sub03/stim_label_session10.mat');
     run_index=[1:6];
     stim_label_session = stim_label(:,run_index);
     clear stim_label;
     
     design={};
     design_total = zeros(num_timepoints,64);
     for p=1:numel(runix)
         %GLMdenoise wants (time)x(conditions)
         %ie: dim1 of design must match dim2 of data 
          design{p}=zeros(num_timepoints,num_conditions);
          design_total = zeros(num_timepoints,64);
          stim_label_session_run = stim_label_session(:,p);
          ...
          %duration later.         
          %totallly 120 conditions, session1 60 conditions, session2 60 conditions
          for num_con = 1:64
                design_total(6*find(stim_label_session_run==num_con),num_con)=1;% tr = 2s; interval = 2points, from the second timepoint(trim the first 9 points for 18 seconds).
          end
          % find where are the 60 conditions are
          design_position = find(sum(design_total)~=0);
          design{p} = design_total(:,design_position);
          % add 1.
          for j=1:size(design{p}, 2)
              index = find(design{p}(:,j) == 1);
              for id = 1:length(index)
                  design{p}(index(id)+1:index(id)+2,j) = 1;
              end
...
% % end
% 
% load([input_dir '/nsdcoreexampledataset.mat']);

% fullfile(input_dir, 'nsdcoreexampledataset.mat');input_file =
...
%         title('Average GLM betas (64 stimuli)')
%    
%     else
